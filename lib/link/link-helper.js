...
class LinkHelper {
  ...
  createLinkToFile() {
    ...
    switch (path.getExt()) {
      ...
      case 'gif':
      case 'bmp':
        return new Link(path, path.fileName(), { isImageLink: true });
      default:
        return new Link(path, path.fileName());
    }
  }

  createLinkToAnchor(anchor) {
    return new Link(getCurrentFilePath(), Header.first().title, {anchor});
  }

  createLinksToAnchors(anchors) {
    return anchors.map(anchor => new Link(getCurrentFilePath(), Header.first().title, {anchor}));
  }
