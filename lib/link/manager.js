...
// Jump history
import { withJumpHistoryRegister, popJumpHistoryStack } from './jump-history';
// TOC
import Toc from './toc';
import TocRegistry from './toc-registry';
// Utilities
import { getCurrentLine, selectRange, getCursorPosition, insertText, getLines } from '../utility/editor/editor';
...
class LinkManager {
  ...
  jumpBackward() {
    // Pop
    const history = popJumpHistoryStack();

    if (history) {
      // Open the file and jump to the point
      File.open(history.filePath).setCursor(history.point);
    }
