...
/**
 * 現在開いているファイルのpathをAdaptedPathとして返します。
 *
 * @return AdaptedPath
 */
export const getCurrentFilePath = () => {
  const path = _getCurrentFilePath();
...
/**
 * '~/hoge/piyo.txt' を '/home/someone/hoge/piyo.txt' などに変換します。
 */
...
class AdaptedPath {
  ...
  constructor(path) {
    ...
    this.homeResolved = path ? resolveHome(path) : _getCurrentFilePath();
